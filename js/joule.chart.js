...
function chart(options) {
    ...
    this.redraw = function(options) { // Refresh a chart
        joule.ui.loading(true);

        joule.data.createPlotData(that.type, options).then(function() {
            switch (that.type) {
                case "line":
                    lineUpdate(options);
                    break;
    ...
    this.tree = function(sensorTree) { // Initialise the tree
        ...
        this.rootData.name = joule.tree.title;
        this.rootData.x0 = 0;
        this.rootData.y0 = 0;
        this.rootData.treeDim = treeDim;
        this.rootData.children = joule.utils.mapper(joule.tree.plotLineDesc);
    ...
    this.treeUpdate = function(options) { // Update the tree
        ...
        joule.utils.updater(that.rootData, joule.tree.plotLineDesc, options.expand); // Update the tree data depending on the selected plot lines
        ...
        nodeEnter.append("svg:rect")
            ...
            .style("fill-opacity", function(d) {
                return d.selected ? 0.65 : (d.children || d._children) ? 0.65 : 0.3;
            })
            .attr("rx", 3)
            .attr("ry", 3)
            .on("click", click)
        ...
        var arrows = nodeEnter.filter(function(d) {
                return d.children || d._children;
            }).append("svg:polygon")
            .attr("class", "arrow")
            .attr("fill", "black")
        ...
        arrowsUpdate.filter(function(d) {
                return d.children;
            })
    ...
    var sliceData = function(range) { // Slices the data so that the focus only shows the selected range. It also filters data by sampling and applying a gaussian filter
        ...
        if (typeof range == 'undefined') {
            ...
            chartTemp.focusData = joule.data.plotData.data;
            chartTemp.fx.domain([joule.data.plotData.start, joule.data.plotData.end]);
        }
        else {
            ...
            var slicedData = [];
            for (var j = 0; j < joule.data.plotData.data.length; j++) {
                slicedData[j] = [];
                var remapped = _.map(_.pluck(joule.data.plotData.data[j].data, "x"), function(d) {
                    return d.getTime();
                }); // Map elements by x property and convert to timestamp
                slicedData[j].data = (joule.data.plotData.data[j].data.slice(
                ...
                slicedData[j].id = joule.data.plotData.data[j].id;
                slicedData[j].avg = joule.data.plotData.data[j].avg;
            }
        ...
        chartTemp.focusData.yMax = d3.max(chartTemp.focusData.data, function(d) {
            return d3.max(d.data, function(e) {
                return typeof e.y === 'number' ? e.y : 0;
            });
        });
        chartTemp.focusData.yMin = d3.min(chartTemp.focusData.data, function(d) {
        ...
        var trimmedData = [];
        if (joule.data.plotData.data.length > 0) {
            // Sample the data for the context if more than 600 points are being displayed
            if (joule.data.plotData.data[0].data.length < 600) {
                for (var i = 0; i < joule.data.plotData.data.length; i++) {
                    trimmedData[i] = new Object();
                    trimmedData[i].id = joule.data.plotData.data[i].id;
                    trimmedData[i].data = [];
                    that.filter ? trimmedData[i].data = gaussian(5, 3, joule.data.plotData.data[i].data) : trimmedData[i].data = joule.data.plotData.data[i].data;
                }
            }
            else {
                var mydiv = joule.data.plotData.data[0].data.length / 600;
                var rounddiv = Math.round(mydiv * 1) / 1;

                for (var p = 0; p < joule.data.plotData.data.length; p++) {
                    for (var q = 0; q < joule.data.plotData.data[p].data.length; q++) {
                        if (q == 0) {
                            trimmedData[p] = {};
                            trimmedData[p].data = [];
                        }
                        if (q % rounddiv == 0) {
                            trimmedData[p].data.push(joule.data.plotData.data[p].data[q]);
                        }
                    }
                    that.filter ? trimmedData[p].data = gaussian(5, 3, trimmedData[p].data) : null;
                    trimmedData[p].id = joule.data.plotData.data[p].id;
                    trimmedData[p].avg = joule.data.plotData.data[p].avg;
                }
            }
        }
    ...
    var line = function() { // Initiliase a line chart
        ...
        chartTemp.color = function(id) {
            ...
            _.each(that.cols, function(v, k, l) {
                _.some(joule.tree.selected, function(d) {
                    return d.id == v;
                }) ? l[k] = v : l[k] = null;
            });
        ...
        chartTemp.focusLine = d3.svg.line()
            ...
            .x(function(d) {
                return chartTemp.fx(d.x);
            })
            .y(function(d) {
                return chartTemp.fy(d.y);
            });
        ...
        chartTemp.xAxisFocusBottom = d3.svg.axis()
            ...
            .tickFormat(function(d) {
                return zoomTimeFormat(new Date(d));
            });
        ...
        chartTemp.xAxisFocusTop = d3.svg.axis()
            ...
            .tickSize(0)
            .tickFormat(function(d) {
                return zoomDateFormat(new Date(d));
            });
        ...
        chartTemp.xAxisContext = d3.svg.axis()
            ...
            .outerTickSize(0)
            .tickFormat(function(d) {
                return overallDateFormat(d);
            });

        chartTemp.contextLine = d3.svg.line()
            .interpolate("monotone")
            .x(function(d) {
                return chartTemp.cx(d.x);
            })
            .y(function(d) {
                return chartTemp.cy(d.y);
            });
	...
	if (joule.data.plotData.data.length < 1) return;
        if (!options.refresh) {
            ...
            chartTemp.cx.domain([joule.data.plotData.start, joule.data.plotData.end]);
            chartTemp.cy.domain([0, joule.data.plotData.yMax]);
            // Check the brush and set its extent to either the intial values or to the same values when the range changes
            if (chartTemp.cBrush.empty() || chartTemp.cBrush.extent()[0].isBefore(joule.data.plotData.start)) {
                var i = {
                    x: joule.config.zoomSize.value.zoomX,
                    dx: joule.config.zoomSize.value.zoomDX
                };
                var d1 = chartTemp.cx.invert(joule.config.zoomSize.value.zoomX);
                joule.config.zoomSize.value.zoomX = i.x;
                var d2 = chartTemp.cx.invert(joule.config.zoomSize.value.zoomX + joule.config.zoomSize.value.zoomDX);
                joule.config.zoomSize.value.zoomDX = i.dx;
            ...
            switch (that.scaleSelection) {
                case 'all':
                    chartTemp.fy.domain([0, joule.data.plotData.yMax * 1.1]);
                    break;
                case 'scale':
            ...
            chartTemp.focus.selectAll(".f-line")
                .data(chartTemp.focusData.data, function(d, i) {
                    return d.id;
                })
                ...
                .append("svg:path")
                .attr("d", function(d) {
                    return chartTemp.focusLine(d.data);
                })
                .attr("class", function(d, i) {
                    return "focusLine" + d.id;
                })
            ...
            chartTemp.focus.selectAll(".f-line").sort(function(a, b) {
                return a.avg < b.avg;
            });
            // Draw focus areas
            chartTemp.focus.selectAll(".f-area")
                ...
                .append("svg:path")
                .attr("d", function(d) {
                    return chartTemp.focusArea(d.data);
                })
                .attr("class", function(d, i) {
                    return "focusArea" + d.id;
                })
                .attr("clip-path", "url(#focusViewport)")
            ...
            // Sort areas as above
            chartTemp.focus.selectAll(".f-area").sort(function(a, b) {
                return a.avg > b.avg;
            });
            ...
            chartTemp.context.selectAll(".c-line")
                .data(chartTemp.contextData.data, function(d, i) {
                    return d.id + "#" + d.data.length;
                })
                .enter().insert("svg:g", ".brush")
                .attr("class", "c-line")
                .append("svg:path")
                .attr("d", function(d) {
                    return chartTemp.contextLine(d.data);
                })
                .attr("class", function(d, i) {
                    return "contextLine" + d.id;
                })
        ...
        _.each(joule.data.plotData.data, function(d, i) {
    ...
    var gridUpdate = function() { // Update or create the data table
        ...
        if (typeof that.grid == 'undefined') {
            ...
            that.grid.setData(joule.data.plotData.table);
	...
	if (jQuery.isEmptyObject(sData)) return;
        ...
        var link = chartTemp.container.links.selectAll(".sLink")
            .data(sData.links, function(d) {
                d.linkId = d.source.node + "-" + d.target.node;
                return d.linkId;
            });
        ...
        nodeEnter.filter(function(d) {
            ...
            .style("font-size", function(d) {
                return ((1 - d.x / dim.w) * 4 + 8) + "px"
            })
            .text(function(d) {
                return d.name;
            })
            .filter(function(d) {
                return d.x < 2 * dim.w / 3;
            })
        ...
        node.call(d3.behavior.drag()
                ...
                .on("dragstart", function() {
                    if (!joule.ui.sankey.playing) this.parentNode.appendChild(this);
                })
                .on("drag", dragmove))
            .on("dblclick", function(d) {
                joule.ui.tree.nodeClick({
                    "id": d.node
                });
            });
        ...
        var textUpdate = nodeUpdate.select("text")
            ...
            .style("font-size", function(d) {
                return ((1 - d.x / dim.w) * 4 + 8) + "px";
            }); // Text size depends on x-position (more nodes to the right), should also depend on number of nodes on the level

        textUpdate.filter(function(d) {
                return d.x < 2 * dim.w / 3;
            })
            .attr("x", 6 + chartTemp.sankey.nodeWidth())
            .attr("text-anchor", "start");
    ...
    var treeMapUpdate = function(options) { // Update the tree map
        ...
        function display(d) {
            ...
            function transition(d) {
                ...
                chartTemp.container.selectAll(".depth").sort(function(a, b) {
        ...
        function nameTag(text) {
            text.attr("x", function(d) {
                    return chartTemp.x(d.x) + 6;
                })
                .attr("y", function(d) {
                    return chartTemp.y(d.y) + 6;
                });
        }

        function valueTag(text) {
            text.attr("x", function(d) {
                    return chartTemp.x(d.x + d.dx) - 6;
                })
                .attr("y", function(d) {
                    return chartTemp.y(d.y + d.dy) - 6;
                });
        }

        function rect(rect) {
            rect.attr("x", function(d) {
                    return chartTemp.x(d.x);
                })
                .attr("y", function(d) {
                    return chartTemp.y(d.y);
                })
                .attr("width", function(d) {
                    return chartTemp.x(d.x + d.dx) - chartTemp.x(d.x);
                })
                .attr("height", function(d) {
                    return chartTemp.y(d.y + d.dy) - chartTemp.y(d.y);
                });
        }
    ...
    var gaussian = function(size, sigma, data) { // Apply a gaussian filter of size and sigma to data
        ...
        _.each(kernel, function(d) {
            normalized.push(d / sum)
        }); // Normalize kernel
